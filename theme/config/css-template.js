...
async function addCssTemplate () {
  try {
    ...
    files.forEach(async (fileName) => {
      if (fileName.includes('jquery') || fileName.includes('main')) {
        console.log('TCL: addFileRevToTemplate -> fileName', fileName)
        mainJs += conf.CSS_LINK.replace('%_file_%', fileName) + '\n'
        console.log('TCL: addFileRevToTemplate -> str', mainJs)
        await fsPromises.writeFile(`${conf.CSS_DIST}main.html`, mainJs)
      } else if ((fileName.includes('plugin1'))) {
        plugin1 = conf.CSS_LINK.replace('%_file_%', fileName)
        await fsPromises.writeFile(`${conf.CSS_DIST}plugin1.html`, plugin1)
      } else {
        plugin2 = conf.CSS_LINK.replace('%_file_%', fileName)
        await fsPromises.writeFile(`${conf.CSS_DIST}plugin2.html`, plugin2)
      }
    })
  } catch (error) {
    console.error('Error occurred:', error)
